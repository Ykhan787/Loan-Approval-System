...
st.set_page_config(page_title="Loan AI Assistant", page_icon="🏦", layout="wide")
st.title("🏦 Smart Loan Approval System")
st.caption("AI-powered loan decision system for banking use")
st.markdown("---")
st.caption("Created by Muhammad Younas | Data Science Batch 5")
...
st.sidebar.header("📂 Dataset Control Panel")
st.sidebar.header("🤖 Model Training Section")

csv_path = st.sidebar.text_input(
    "CSV Path",
    value = "loan_dataset.csv",
    help = "Put the path to the dataset CSV. if you run from the same folder, keep it as-is"
)
...
with colB:
    ...
    with st.expander("📘 How This Model Works"):
        st.write("""This system uses **Logistic Regression** to predict loan approval.It analyzes income, credit score, employment type,and previous loan history to calculate approval probability.""")
...
if st.button("Predict Approval"):
    ...
    pred = int(prob >= APPROVAL_THRESHOLD)
    st.caption("Approval threshold set at 60% confidence")

    if pred == 1 :
        st.success(f"✅ {applicant_name} : APPROVED ({prob:.2%})")
    else: 
        st.error(f"❌ {applicant_name} : REJECTED ({prob:.2%})")
